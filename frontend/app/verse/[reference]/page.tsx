...
export default function VersePage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center px-6 py-20" style={{ background: '#0A0F2E' }}>
      <div className="max-w-2xl w-full border border-saffron/20 rounded-2xl p-8" style={{ background: '#0D1225' }}>
        <div className="flex items-center gap-2 mb-6 flex-wrap">
          ...
          {(verse.themes || []).slice(0, 2).map((t) => (
            <span key={t} className="px-2 py-1 border border-saffron/20 text-cream/50 text-xs rounded-full capitalize">
              {t}
            </span>
          ))}
        </div>

        <p className="text-cream text-2xl leading-relaxed mb-4 text-center" style={{ fontFamily: 'serif' }} lang="sa">
          {verse.sanskrit_text}
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <p className="text-cream/80 leading-relaxed text-sm" lang="hi">{verse.hindi_meaning}</p>
          </div>
          <div>
