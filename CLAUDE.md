...
### Frontend
- **Sign-in gate:** `app/layout.tsx` wraps every route in `AuthProvider` + `AuthGate` (`components/auth/`) — seekers sign in with an **email one-time code** (Supabase OTP; the same email's magic link also works) before the app renders. Fail-open by design: without the Supabase env vars the gate is invisible and the app runs open, so zero-env dev still works. Phone/SMS was intentionally dropped. Supabase dashboard requirements: the *Magic Link* email template must include `{{ .Token }}` for the code to appear, and the site URL must be whitelisted under Auth → URL Configuration.
