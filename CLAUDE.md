...
- `GET /api/verse-insight?ref=2.47` — the "Go deeper" study companion for one verse (essence / dialogue context / modern analogy / practice / reflection). AI overlay (Vertex → Gemini) with a deterministic per-verse template fallback (`lib/verseInsight.ts`); AI results cached in-memory and the route is CDN-cached for a day. Never hard-fails.
- `GET /api/stories` — Mahabharata stories from `data/stories.json` (each story ships a short description **plus** a full hand-written `narration` sized for one TTS call, a `lesson`, and `gita_refs` verse anchors — StoryCards narrates via `lib/useTTS`: server Gemini voice → browser fallback).
...
### The Darshan layer (hero visual identity)

The home hero is a deep-indigo **darshan**, not a normal landing section: a cosmic sky (`CosmicBackdrop`), Madhav seen through an arched window of light, and the shloka resting on glass with its own reflection. Three rules govern everything in `components/darshan/`:

1. **The Sudarshan Chakra is the brand.** `SudarshanChakra` is pure SVG driven by *one* rAF loop that eases rotation speed and a `--chakra-glow` variable toward the target for the current `ChakraState` (`idle` / `processing` / `settling` / `still`) — momentum, never a snap. It is deliberately not a CSS animation, because `animation-duration` cannot be interpolated. It doubles as the navbar logo.
2. **Motion is spent, not sprayed.** The launch ritual (`ChakraLaunch`: chakra gathers light → expanding ring → flies into the logo → veil dissolves) plays **once per session** and is skippable by click, the Skip button, or Escape. The mor pankh drifts once per load. Nothing loops on interaction.
3. **Every motion has a still counterpart.** `lib/motion.ts` is the single gate: under `prefers-reduced-motion` the ritual never plays, the feathers become one still feather, the sky stops turning, and the verse reflection is dropped. Fail-open is non-negotiable — the hero also self-reveals on a 3.5s failsafe if the ritual never reports back, so a blocked animation can never leave a blank page.

The ritual and the page it unveils talk only through `lib/darshan-launch.ts` (`madhav:darshan-ready` + `data-chakra-logo`), mirroring the existing `madhav:prefill` / `madhav:voice` event-bus idiom — no state threaded through the tree, and the flight target is measured live, so the logo can move freely.

**The Darshan Experience Engine** (`lib/darshan/*` + `components/darshan/DarshanProvider.tsx`) sits above all of that and conducts it. It owns three things and nothing else: a 12-state machine (`loading → entering → idle → thinking → answering → blessing → …`, with an explicit transition table and a *terminal* `reduced_motion`), a device/preference motion tier (`full` / `lite` / `still`), and the resolved config + active quote. Visual layers read the engine rather than running their own clocks — which is why the navbar chakra turns when a question is asked three sections down the page. Anything can drive it without importing it: `darshan.thinking('chat')` over the `madhav:darshan` bus.

The tier decides whether WebGL is ever downloaded: the R3F scenes (`components/darshan/three/*`) are behind dynamic imports only reached at `full` tier, so three.js stays out of the initial payload (verified against the production build). The hero's presence is **aniconic by design** — light, not likeness, continuing `MadhavLight` and the safety constraint — and every vertex is generated in code, so no asset in the engine can be unlicensed. Vishwaroop is an opt-in, time-bounded, deliberately abstract reveal, never an ambient state.

Content that carries spiritual or legal weight is data, not code: `supabase/darshan-schema.sql` defines the CMS tables (with constraints that refuse unreviewed forms and unlicensed models), and `lib/darshan/config.ts` is the local fallback. **Full guide: [`docs/DARSHAN.md`](./docs/DARSHAN.md)** — read it before changing motion, adding a form, or replacing placeholder assets.

### Frontend
- **Sign-in gate:** `app/layout.tsx` wraps every route in `AuthProvider` + `AuthGate` (`components/auth/`) — seekers sign in with an **email one-time code** (Supabase OTP; the same email's magic link also works) before the app renders. Fail-open by design: without the Supabase env vars the gate is invisible and the app runs open, so zero-env dev still works. Phone/SMS was intentionally dropped. `NEXT_PUBLIC_REQUIRE_SIGNIN=false` is a launch kill-switch that keeps the gate dormant while Supabase is configured. **Sign-in emails are sent by the app itself**, not Supabase's dev-only mailer: Supabase's Auth *Send Email Hook* posts to `/api/auth-email`, which verifies the webhook signature and sends a branded email (one-time code + link) via Resend (`SEND_EMAIL_HOOK_SECRET`, `RESEND_API_KEY`, `AUTH_EMAIL_FROM`). Supabase dashboard: Site URL + redirect allow-list under Auth → URL Configuration (done), and the Send Email Hook pointed at the endpoint.
...
| `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY` | The app-wide email-OTP sign-in gate **and** the Sankalpa Journal (saved verses, streaks). Absent → app runs open and the Journal is inert. |
